...
describe('Mastodon Node - Instance', () => {
	let node: Mastodon;
	...
	it('should get server info', async () => {
		const mockInfo = { title: 'Mastodon', version: '4.2.0' };
		(ctx.getNodeParameter as jest.Mock).mockImplementation((param) => {
			if (param === 'resource') return 'instance';
			if (param === 'operation') return 'getServerInfo';
			return undefined;
		});
		(ctx.helpers!.requestOAuth2 as jest.Mock).mockResolvedValue(mockInfo);
		...
		expect(ctx.helpers!.requestOAuth2).toHaveBeenCalledWith(
			...
			expect.objectContaining({
				method: 'GET',
				uri: expect.stringContaining('/api/v2/instance'),
			}),
		);
