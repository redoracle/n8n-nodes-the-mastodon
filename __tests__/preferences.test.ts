...
describe('Mastodon Node - Preferences', () => {
	let node: Mastodon;
	...
	it('should get preferences', async () => {
		const mockPrefs = { posting: 'public' };
		(ctx.getNodeParameter as jest.Mock).mockImplementation((param) => {
			if (param === 'resource') return 'preferences';
			if (param === 'operation') return 'get';
			return undefined;
		});
		(ctx.helpers!.requestOAuth2 as jest.Mock).mockResolvedValue(mockPrefs);
		...
		expect(ctx.helpers!.requestOAuth2).toHaveBeenCalledWith(
			...
			expect.objectContaining({
				method: 'GET',
				uri: expect.stringContaining('/api/v1/preferences'),
			}),
		);
		expect(result[0][0].json).toEqual(mockPrefs);
	});
});
